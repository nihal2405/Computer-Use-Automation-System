...
from scripts.acceptance import SOURCE_FILES, snapshot, test_summary as summarize
...
def test_source_snapshot_excludes_credentials_environments_and_generated_files(tmp_path):
    ...
    for name in (".env", ".venv/private", "runs/private", "src/__pycache__/private.pyc", "src/.env.local"):
        path = source / name
    ...
    assert not any("do-not-export" in p.read_text() for p in (tmp_path / "copy").rglob("*") if p.is_file())
