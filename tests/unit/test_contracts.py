...
from computer_use.schemas import (
    Capability, ControlSnapshot, ControlTransition, Intervention, Observation,
    TargetIdentity, action_adapter, result_adapter, target_adapter,
)
...
def test_development_artifact_round_trip_and_invocation(artifact):
    ...
    capability.validate_target(TargetIdentity.model_validate_json(
        (FIXTURES / "mock_bank_identity.json").read_text()
    ))
...
@pytest.mark.parametrize("path", [
    "https://unapproved.example/", "//unapproved.example/", "javascript:alert(1)",
    "/../admin", "/%2e%2e/admin", "/%2fexample.com", "/%252fexample.com",
    "/\\example.com", "/?token=secret", "/#fragment", "/\nadmin", "relative/path",
])
...
@pytest.mark.parametrize("inputs", [{}, {"member_id": 123}, {"member_id": True},
                                       {"member_id": ""}, {"member_id": "1001", "extra": "x"}, []])
...
@pytest.mark.parametrize("outputs", [
    {}, {"balance": "1.00"}, {"balance": 1.0, "currency": "USD"},
    {"balance": "NaN", "currency": "USD"}, {"balance": "1e3", "currency": "USD"},
    {"balance": "1,000.00", "currency": "USD"}, {"balance": "1.00", "currency": "usd"},
...
@pytest.mark.parametrize(("kind", "valid", "invalid"), [
    ("integer", 42, True), ("boolean", False, 0), ("string", "00123", 123),
    ("decimal_string", "-12.50", "Infinity"), ("currency_code", "EUR", "euro"),
])
...
@pytest.mark.parametrize(("field", "value"), [
    ("product", "other_bank"), ("version", "2.0"), ("surface", "desktop"),
])
...
def test_actions_and_targets_round_trip(artifact):
    ...
    actions += [{"action": "wait", "condition": condition, "timeout_ms": 1000},
...
@pytest.fixture
def run_context():
    return {"schema_version": "1.0", "run_id": "run_1", "session_id": "session_1", "mode": "replay",
            "capability_id": "read_savings_balance", "capability_version": "1.0.0"}


def test_results_are_distinct_and_bound_to_capability(artifact, run_context):
    capability = Capability.model_validate(artifact)
    results = [
        {**run_context, "status": "success", "checkpoint_verified": True,
         "outputs": {"balance": "1250.75", "currency": "USD"}},
        {**run_context, "status": "business_outcome", "code": "member_not_found",
         "step_id": "submit_search", "summary": "No matching member"},
        {**run_context, "status": "failure", "code": "permission_denied",
         "diagnostic": {"step_id": "open_accounts", "expected": "Accounts visible",
                        "observed": "Permission denied", "evidence_ref": "sanitized_snapshot_1"}},
    ]
    ...
    for path, value in [(("outputs", "extra"), "bad"), (("outputs", "balance"), "NaN"),
                        (("capability_id",), "another_flow"), (("capability_version",), "2.0.0")]:
        changed = deepcopy(results[0])
...
def test_discovery_can_fail_before_a_capability_exists():
    failure = {
        "schema_version": "1.0", "run_id": "run_1", "session_id": "session_1",
        "mode": "discovery", "status": "failure", "code": "timeout",
...
def observation():
    return {
        "schema_version": "1.0", "run_id": "run_1", "session_id": "session_1",
        "sequence": 3, "target": {"product": "synthetic_bank", "version": "1.0", "surface": "browser"},
        "location": "/members", "summary": "A confirmation dialog blocks navigation",
        "state": "unexpected_dialog", "controls": [], "evidence_ref": "sanitized_snapshot_1",
    }


def test_observation_rejects_duplicate_ids_unbound_targets_and_objects(observation, artifact):
    control = {"id": "search", "target": artifact["steps"][2]["action"]["target"],
...
def intervention(observation):
    return {
        "schema_version": "1.0", "id": "intervention_1", "run_id": "run_1",
        "session_id": "session_1", "goal": "Read a synthetic savings balance",
        "capability_id": "read_savings_balance", "capability_version": "1.0.0",
        "step_id": "open_accounts", "reason": "unknown_state", "context": observation,
        "diagnostic": {"step_id": "open_accounts", "expected": "Accounts visible",
...
def test_intervention_open_and_resolved_round_trip(intervention):
    ...
    intervention["resolution"] = {
        "action": "resume", "operator_id": "operator_1", "summary": "Dismissed dialog",
        "human_actions_ref": "sanitized_human_events_1", "state_verified": True,
        "verification": intervention["diagnostic"], "resume_step_id": "open_accounts",
    }
...
@pytest.mark.parametrize(("action", "state", "verified"), [
    ("complete", "COMPLETED", True), ("abort", "FAILED", False),
])
def test_intervention_can_complete_or_abort(intervention, action, state, verified):
    ...
    intervention["resolution"] = {
        "action": action, "operator_id": "operator_1", "summary": "Resolved intervention",
        "human_actions_ref": "sanitized_human_events_1", "state_verified": verified,
...
@pytest.mark.parametrize(("path", "value"), [
    (("context", "run_id"), "another_run"), (("context", "session_id"), "another_session"),
    (("control", "session_id"), "another_session"), (("control", "owner"), "automation"),
    (("diagnostic", "step_id"), "another_step"), (("capability_version",), None),
...
def test_control_transfer_requires_same_session_and_resume_check():
    states = [
        ...
        ("HUMAN_CONTROL", "human"), ("RESUME_CHECK", "automation"), ("COMPLETED", "none"),
    ]
    ...
    assert [snapshot.automation_actions_allowed for snapshot in snapshots] == [True, False, False, False, False]
    for before, after in [(snapshots[2], snapshots[0]), (snapshots[4], snapshots[0]),
                          (snapshots[1], snapshots[0])]:
        with pytest.raises(ValidationError):
            ControlTransition(before=before, after=after, reason="Invalid transition")
    with pytest.raises(ValidationError):
        ControlTransition(before=snapshots[0], after=ControlSnapshot(
...
def test_cli_validates_fixture_without_execution_and_hides_bad_values(tmp_path):
    command = [sys.executable, "-m", "computer_use", "validate-capability",
               str(FIXTURES / "read_savings_balance.json")]
    completed = subprocess.run(command + ["--inputs", '{"member_id":"1001"}', "--target",
    ...
    completed = subprocess.run(command[:4] + [str(bad)], capture_output=True, text=True, check=False)
    assert completed.returncode == 2
    assert json.loads(completed.stdout)["status"] == "invalid"
    assert secret not in completed.stdout + completed.stderr


@pytest.mark.parametrize("inputs", ["null", "[]", "invalid json", '{"member_id":123}',
                                   '{"member_id":"1001","secret":"SHOULD_NOT_PRINT"}'])
def test_cli_rejects_malformed_invocations_without_tracebacks(inputs):
    completed = subprocess.run(
        [sys.executable, "-m", "computer_use", "validate-capability",
         str(FIXTURES / "read_savings_balance.json"), "--inputs", inputs],
        capture_output=True, text=True, check=False,
    )
    assert completed.returncode == 2
    assert json.loads(completed.stdout)["status"] == "invalid"
