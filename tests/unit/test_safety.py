...
@pytest.mark.parametrize(
    "url",
    [
        "http://127.0.0.1:8000/%252e%252e/transfer",
        "http://127.0.0.1:8000/../transfer",
        "http://user:password@127.0.0.1:8000/",
        "javascript:alert(1)",
        "http://127.0.0.1:8000/ members",
        "http://127.0.0.1:8000//members",
        "http://127.0.0.1:8000/members?token=secret",
...
def test_redaction_covers_unknown_text_keys_numbers_and_trusted_secret_collisions(tmp_path):
    ...
    store = EvidenceStore(
        tmp_path, redactor, run_id=str(uuid4()), session_id=str(uuid4()), mode="replay"
    )
    for kind in (
        "error",
        "human_event",
...
@pytest.mark.parametrize(
    "values", [{}, {"member_id": 1001}, {"member_id": "abc"}, {"member_id": "1001", "secret": "no"}]
)
...
def test_artifact_export_preserves_valid_workflow_without_sensitive_prose(tmp_path):
    ...
    text = path.read_text()
    assert all(
        v not in text for v in ("Private Person", "private@example.com", "token-super-secret")
    )
    ...
    data["steps"][3]["action"]["target"]["name"] = {
        "source": "literal",
        "value": "secret-member-identifier",
    }
