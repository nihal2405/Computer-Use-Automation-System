...
ARTIFACT = Path(
    os.environ.get("REPLAY_TEST_ARTIFACT", str(ROOT / "tests/fixtures/read_savings_balance.json"))
)
...
def engine(bank, tmp_path, member="2002", capability=None, **settings):
    return ReplayEngine(
        capability=capability or CapabilityStore.load(ARTIFACT),
...
def events(replay):
    return [
        json.loads(line)
        for line in (replay.executor.store.directory / "events.jsonl").read_text().splitlines()
    ]
...
@async_test
async def test_same_artifact_binds_new_inputs_and_validates_visible_outputs(
    bank, tmp_path, member, balance
):
    ...
    async with engine(bank, tmp_path, member) as replay:
        ...
        assert all(
            secret not in persisted(replay)
            for secret in (member, balance, "Jordan Ellis", "Avery Morgan")
        )
...
async def test_exhaustion_emits_valid_sanitized_intervention_and_retains_session(bank, tmp_path):
    ...
    async with engine(bank, tmp_path, capability=Capability.model_validate(data)) as replay:
        ...
        assert (
            await replay.executor.execute_step(replay.capability.steps[2])
...
async def test_application_states_do_not_become_success(bank, tmp_path, scenario, code):
    ...
    async with engine(bank, tmp_path) as replay:
        ...
        assert not any(
            e["action"] == "read" and e["event"] in {"started", "completed"} for e in events(replay)
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ('class="accounts-member-id">2002', 'class="accounts-member-id">9999'),
            "checkpoint_failed",
        ),
...
@async_test
async def test_wrong_owner_invalid_output_unknown_state_and_version(
...
@async_test
async def test_persistence_failure_while_reporting_surface_error_is_fail_closed(
...
def test_cli_runs_in_separate_process_with_model_imports_blocked(bank, tmp_path):
    ...
    for path, value in [
        ("settings.yaml", config.runtime),
        ("policy.yaml", config.policy),
        ("targets/mock_bank.yaml", config.target),
    ]:
