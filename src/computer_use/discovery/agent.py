...
HARD_STATES = {"validation_error": "invalid_input", "permission_denied": "permission_denied",
...
class DiscoveryAgent:
    def __init__(self, *, task, configuration, project_root, inputs, model):
        self.task = DiscoveryTask.model_validate_json(DiscoveryTask.model_validate(task).model_dump_json())
        ...
        if (target.product != self.task.target.product or target.surface != self.task.target.surface
        ...
        for condition in [self.task.success_checkpoint, *[r.when for r in self.task.business_outcomes],
        ...
        self.executor = Executor(configuration=self.config, project_root=project_root, mode="discovery", inputs=self.inputs)
        self.model = model
    ...
    def from_project(cls, root, *, task_path, inputs, model, configuration=None, goal=None):
        ...
        return cls(task=task, configuration=configuration or load_configuration(root),
                   project_root=root, inputs=inputs, model=model)
    ...
    def _candidates(self):
        ...
        for rule in self.config.policy.rules:
            ...
            if rule.action == "navigate":
                ...
                action["path"] = entry
            if rule.action == "read":
                names = [name for name, target in self.task.output_targets.items()
            ...
            candidates.append(Step(id=f"candidate_{len(candidates)}", operation=rule.operation, action=action))
        if {s.action.output for s in candidates if s.action.action == "read"} != set(self.task.outputs):
        ...
        return sorted(candidates, key=lambda step: json.dumps(step.action.model_dump(), sort_keys=True))
    ...
    def _run_context(self):
        return dict(schema_version="1.0", run_id=self.executor.run_id,
                    session_id=self.executor.session_id, mode="discovery")
    ...
    def _event(self, event, **details):
        self.executor.store.event(event=event, step_id=self._last_step.id if self._last_step else None,
            control_state=self.executor.control.state, details=details)

    async def _finish(self, result):
        self._event({"success": "run_completed", "failure": "run_failed", "business_outcome": "business_outcome"}[result.status],
    ...
    async def _stop(self, code, diagnostic=None):
        ...
        if diagnostic is None:
            failed = await self.executor.report_failure(self._last_step, code,
                "Verified progress within approved discovery limits", "Discovery stopped: " + code)
            diagnostic = failed.diagnostic
        intervention_id = None
        if code != "invalid_input":
            context = self._context or Observation(schema_version="1.0", run_id=self.executor.run_id,
                session_id=self.executor.session_id, sequence=0,
                target=TargetIdentity(product=self.config.target.product, version=self.config.target.supported_versions[0], surface="browser"),
                location="unavailable", summary="UI unavailable", controls=[], state="unknown")
            try:
                ...
                reason = code if code in {"timeout", "no_progress", "step_limit", "recovery_exhausted", "permission_denied", "session_expired"} else ("policy_blocked" if code == "policy_denied" else "unknown_state")
                self.intervention = self.executor.save_intervention(Intervention(
                    schema_version="1.0", id=intervention_id, **{k: v for k, v in self._run_context().items() if k in {"run_id", "session_id"}},
                    goal=self.task.goal, step_id=diagnostic.step_id, reason=reason,
                    diagnostic=diagnostic, context=context, control=self.executor.control))
        else:
            await self.executor.transition("FAILED", "Invalid input")
        result = Failure(**self._run_context(), status="failure", code=code,
            diagnostic=diagnostic, intervention_id=intervention_id)
    ...
    async def resume_verified(self, start):
        ...
        for step in self.candidates:
            ...
            try:
                self.outputs[step.action.output] = extract_value(self.task.outputs[step.action.output], outcome.output)
            except ValueError:
                return await self._stop("invalid_output")
        terminal = await self._gate()
        if terminal:
            return terminal
        checkpoint = Step(id="completion_check", operation=self.policy.condition_operation(self.task.success_checkpoint, self.inputs),
        ...
        return await self._finish(Success(**self._run_context(), status="success", checkpoint_verified=True, outputs=self.outputs))
            ...
            for rule in self.task.business_outcomes:
                if await self.executor.check_condition(rule.when):
                    ...
                    return await self._finish(BusinessOutcome(**self._run_context(), status="business_outcome",
                        code=rule.code, step_id=self._last_step.id, summary="Declared business outcome observed"))
            recovered = False
            for rule in self.task.recovery_rules:
                ...
                step = Step(id=f"recovery_{len(self.recorder.steps)}", operation=operation,
                            action={"action": "wait", "condition": rule.until, "timeout_ms": rule.timeout_ms}, timeout_ms=rule.timeout_ms)
    ...
    async def _observation(self):
        ...
        for step in self.candidates:
            ...
            actions.append({"operation": step.operation, "action": step.action.model_dump(mode="json"), **preview})
        return {"state": self._context.state, "permitted_actions": actions,
    ...
    async def _complete(self):
        ...
        capability = compile_capability(self.task, self.recorder, run_id=self.executor.run_id,
            identity=self._context.target, outputs=self.outputs, checkpoint_verified=True,
        ...
        ReplayEngine(capability=capability, configuration=self.config, project_root=self.project_root, inputs=self.inputs)
        self.artifact_path = self.executor.artifacts.save(capability)
        self.capability = CapabilityStore.load(self.artifact_path)
        self._event("artifact_created", capability_id=self.capability.id,
        ...
        return await self._finish(Success(**self._run_context(), capability_id=self.capability.id,
            capability_version=self.capability.capability_version, status="success", checkpoint_verified=True, outputs=self.outputs))
    ...
    async def _loop(self):
        ...
        for iteration in range(self.config.runtime.max_steps):
            ...
            context = {"goal": self.task.goal, "input_references": [f"inputs.{k}" for k in self.task.inputs],
            ...
            try:
                ...
                if error.code != "invalid_model_response":
                    self.provider_failure = {"status": error.provider_status, "reason": error.provider_reason}
                ...
                continue
            self._event("model_response", response_id=reply.response_id, model=reply.model, requested_step=decision)
        ...
        try:
            ...
            self.result = Failure(**self._run_context(), status="failure", code="persistence_failed",
                diagnostic=Diagnostic(step_id=self._last_step.id if self._last_step else None,
                                      expected="Sanitized evidence stored", observed="Persistence failed; browser closed"))
            return self.result
