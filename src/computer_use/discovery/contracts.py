...
from computer_use.schemas.capability import (
    BusinessOutcomeRule,
    Step,
    TargetCompatibility,
    WaitRecovery,
...
class DiscoveryTask(Contract):
    ...
    def connected(self) -> Self:
        ...
        if any(
            ref.ref.removeprefix("inputs.") not in self.inputs for ref in input_references(self)
        ):
            raise ValueError("Undeclared task input reference")
        for reference in input_references(self):
            if self.inputs[reference.ref.removeprefix("inputs.")].type not in {
                "string",
                "decimal_string",
                "currency_code",
            }:
