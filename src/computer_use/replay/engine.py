...
class ReplayEngine:
    ...
    def __init__(self, *, capability, configuration: Configuration, project_root: Path, inputs):
        try:
            self.capability = Capability.model_validate_json(
                Capability.model_validate(capability).model_dump_json()
            )
        ...
        if (
            self.capability.target.product != target.product
            or self.capability.target.surface != target.surface
        ...
        try:
            ...
            if (
                self.capability.steps[0].action.action != "navigate"
                or self.capability.steps[0].action.path != entry_path
            ):
            ...
            for step in self.capability.steps:
                ...
                for condition in (step.precondition, step.postcondition):
                    if condition is not None:
                        bound = action_adapter.validate_python(
                            {
                                "action": "verify",
                                "condition": bind_references(condition, self._inputs),
                            }
                        )
                        policy.authorize(
                            bound, step.operation, self._inputs, target.entry_url, origin
                        )
    ...
    async def _inspect(self):
        ...
        try:
            ...
            raise SurfaceError(
                "incompatible_target",
                "Artifact-compatible live target",
                "Visible target version is incompatible",
            ) from None
        return observation
    ...
    async def _fail(
        ...
        code,
        *,
        expected="A verified workflow outcome",
        observed="Replay cannot safely continue",
        diagnostic=None,
    ):
            ...
            try:
                ...
                context = Observation(
                    ...
                    target=TargetIdentity(
                        product=self._configuration.target.product,
                        version=self._configuration.target.supported_versions[0],
                        surface="browser",
                    ),
                    location="unavailable",
                    summary="Current UI observation unavailable",
                    controls=[],
                    state="unknown",
                )
            try:
                await self.executor.transition(
                    "AWAITING_HUMAN", "Replay requires review before further actions"
                )
            except (
                SurfaceError,
                ValueError,
            ):  # Closed or externally controlled sessions cannot offer this takeover.
                pass
            else:
                record_id = str(uuid4())
                reason = (
    ...
    async def _recover(self, rule, step):
        ...
        wait_step = Step(
            id=step.id,
            operation=operation,
            action={"action": "wait", "condition": rule.until, "timeout_ms": rule.timeout_ms},
        ...
        return await self._fail(
            step,
            "recovery_exhausted",
            expected="Recovery condition within the bounded wait budget",
    ...
    async def _gate(self, step):
        while True:
            ...
            if observation.state in _HARD_STATES:
                return await self._fail(
                    step, _HARD_STATES[observation.state], observed=observation.state
                )
            for rule in self.capability.business_outcomes:
                if await self.executor.check_condition(rule.when, step_id=step.id):
                    await self.executor.transition(
                        "COMPLETED", "Expected business outcome observed"
                    )
                    return await self._finish(
                        BusinessOutcome(
                            **self._run_context(),
                            ...
                            code=rule.code,
                            step_id=step.id,
                            summary="Configured business outcome observed in the UI",
                        )
                    )
            recovering = False
            for rule in self.capability.recovery_rules:
            ...
            if observation.state != "ready":
                ...
                return await self._fail(
                    step,
                    "unknown_state",
                    observed="State has no matching approved outcome or recovery rule",
                )
            return None
        ...
        try:
            for index, step in enumerate(self.capability.steps):
                ...
                if step.action.action == "read":
                    try:
                        outputs[step.action.output] = extract_value(
                            self.capability.outputs[step.action.output], outcome.output
                        )
                    except ValueError:
                        return await self._fail(
                            step,
                            "invalid_output",
                            expected="Visible output matching its declared type",
            ...
            if terminal:
                return terminal
            checkpoint = Step(
                id=self._last_step.id,
                operation=self._policy.condition_operation(
                    self.capability.success_checkpoint, self._inputs
                ),
            ...
            if outcome.status == "failure":
                return await self._fail(
                    self._last_step, outcome.code, diagnostic=outcome.diagnostic
                )
            try:
                ...
                return await self._fail(
                    self._last_step,
                    "invalid_output",
                    observed="Final outputs do not satisfy the declared contract",
                )
            ...
            try:
                return await self._fail(
                    self._last_step, error.code, expected=error.expected, observed=error.observed
                )
    ...
    async def _persistence_failure(self):
        ...
        self.result = Failure(
            **self._run_context(),
            ...
            diagnostic=Diagnostic(
                step_id=self._last_step.id if self._last_step else None,
                expected="Sanitized audit evidence stored",
