"""Inspect implementation status and validate development capability contracts."""
...
from computer_use.schemas import Capability, TargetIdentity
from computer_use.settings import ConfigurationError, load_configuration


def main() -> int:
    parser = argparse.ArgumentParser(description="Computer-use automation development tools")
    parser.add_argument("--version", action="version", version=__version__)
    commands = parser.add_subparsers(dest="command", required=True)
    commands.add_parser("status", help="Show implementation status")
    config = commands.add_parser("validate-config", help="Validate trusted runtime, target, and policy YAML")
    config.add_argument("--project", type=Path, default=Path.cwd())
    replay = commands.add_parser("replay", help="Replay a capability through the UI without a model")
    ...
    replay.add_argument("--headless", action="store_true", help="Run without a visible browser window")
    discover = commands.add_parser("discover", help="Discover and compile a workflow using a real model")
    discover.add_argument("--task", type=Path, default=Path("config/tasks/read_savings_balance.json"))
    ...
    discover.add_argument("--goal", help="Goal text; outputs, target and checkpoint remain task-defined")
    discover.add_argument("--headless", action="store_true")
    for command in (replay, discover):
        command.add_argument("--interactive", action="store_true", help="Keep a blocked browser alive for local operator takeover")
        command.add_argument("--operator-timeout", type=int, default=900, help="Maximum operator wait in seconds (1–3600)")
    validate = commands.add_parser("validate-capability", help="Validate a capability without executing it")
    ...
    if args.command == "discover":
        ...
        async def discover_run():
            ...
            try:
                ...
                agent = DiscoveryAgent.from_project(args.project, task_path=task_path,
                    inputs=json.loads(args.inputs), model=model, configuration=configuration, goal=args.goal)
                if args.interactive:
        ...
        try:
            ...
            print(json.dumps({"result": result.model_dump(mode="json"),
                ...
                "session_retained": False}, indent=2))
    ...
    if args.command == "replay":
        ...
        try:
            ...
            engine = ReplayEngine(capability=capability, configuration=configuration,
                                  project_root=args.project, inputs=json.loads(args.inputs))
        except (ArtifactError, ReplayRequestError, ConfigurationError, ValueError, TypeError) as error:
            print(json.dumps({"status": "failure", "code": getattr(error, "code", "invalid_input"), "executed": False}))
            return 1
        try:
            async def run():
                if args.interactive:
            ...
            print(json.dumps({"provenance": capability.provenance, "model_used": False,
                              "session_retained": False, "result": result.model_dump(mode="json")}, indent=2))
            return {"success": 0, "business_outcome": 2, "failure": 1}[result.status]
        except (PersistenceError, BrowserError, OSError, ValueError, TypeError):
            print(json.dumps({"status": "failure", "code": "replay_runtime_or_setup_failed", "model_used": False}))
            return 1
    if args.command == "validate-config":
        try:
            ...
            return 2
        print(json.dumps({"status": "valid", "policy_rules": len(configuration.policy.rules), "executed": False}))
        return 0
    if args.command == "validate-capability":
        try:
            capability = Capability.model_validate_json(args.path.read_text(encoding="utf-8"))
            if args.inputs is not None:
                capability.validate_inputs(json.loads(args.inputs))
            if args.target is not None:
                capability.validate_target(TargetIdentity.model_validate_json(args.target.read_text(encoding="utf-8")))
            ...
            return 2
        print(json.dumps({
            "status": "valid", "capability_id": capability.id,
            "provenance": capability.provenance, "steps": len(capability.steps),
            "inputs_checked": args.inputs is not None, "target_checked": args.target is not None,
            "executed": False,
        }, indent=2))
        return 0
    print(json.dumps({
