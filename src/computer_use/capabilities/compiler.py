...
def compile_capability(task, recorder, *, run_id, identity, outputs, checkpoint_verified, live_provider):
    ...
    return Capability(
        schema_version="1.0", id=task.id, capability_version=task.capability_version,
        ...
        inputs=task.inputs, outputs=task.outputs, steps=recorder.steps,
        success_checkpoint=task.success_checkpoint,
        business_outcomes=task.business_outcomes, recovery_rules=task.recovery_rules,
    )
