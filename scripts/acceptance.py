...
new operating system or a fresh Git clone of this uncommitted local repository.
"""
...
SOURCE_FILES = ("pyproject.toml", "uv.lock", ".python-version", ".env.example", "README.md", "REPORT.md", ".gitignore")
...
def snapshot(source, destination):
    ...
    for path in sorted(selected):
        ...
        if any(part.startswith(".") or part == "__pycache__" or part.endswith(".egg-info") for part in relative.parts[:-1]):
            continue
...
def test_summary(path):
    ...
    return {"total": len(cases), "failed": len(failed), "skipped": len(skipped),
            "passed": len(cases) - len(failed) - len(skipped), "by_module": counts,
...
def main():
    ...
    report = {"schema_version": "1.0", "started_at": datetime.now(timezone.utc).isoformat(),
              "status": "running", "clean_environment": {"fresh_source_copy": True, "fresh_venv": True,
                  "dotenv_copied": False, "existing_run_data_copied": False,
                  "download_caches_reused": True, "new_os": False, "git_clone": False},
              "platform": platform.system(), "architecture": platform.machine(), "stages": []}
    def save():
        (output / "acceptance.json").write_text(json.dumps(report, indent=2) + "\n")
    save()
    env = {k: v for k, v in os.environ.items() if not k.endswith("_API_KEY") and k not in {
        "PYTHONPATH", "PYTHONHOME", "VIRTUAL_ENV", "UV_PROJECT_ENVIRONMENT", "REPLAY_TEST_ARTIFACT", "PYTEST_ADDOPTS"}}
    ...
    def stage(name, command, *, extra_env=None, timeout=600, junit=None):
        ...
        try:
            process = subprocess.run(command, cwd=checkout, env={**env, **(extra_env or {})},
                                     capture_output=True, text=True, timeout=timeout)
            result = {"name": name, "exit_code": process.returncode, "passed": process.returncode == 0}
            if junit and junit.exists():
                result["tests"] = test_summary(junit)
                result["passed"] &= bool(result["tests"]["total"]) and not result["tests"]["failed"] and not result["tests"]["skipped"]
            elif junit:
                result["passed"] = False
    ...
    try:
        ...
        versions = stage("installed_versions", [python, "-c", "import importlib.metadata,json; print(json.dumps({d.metadata['Name']:d.version for d in importlib.metadata.distributions()}))"])
        ...
        for label, paths, extra in [
            ("full_suite", ["tests"], {}),
            ("genuine_artifact_replay_and_handoff", ["tests/integration/test_replay.py", "tests/integration/test_handoff.py"],
             {"REPLAY_TEST_ARTIFACT": str(checkout / "evidence/phase7/capability.json")}),
        ]:
            xml = workspace / (label + ".xml")
            stage(label, [python, "-m", "pytest", "-q", *paths, "--junitxml=" + str(xml)], extra_env=extra, junit=xml)
        stage("real_cli_scenarios", [python, "scripts/capture_acceptance.py", str(output / "scenarios")])
        report["artifact_sha256"] = digest(checkout / "evidence/phase7/capability.json")
        report["status"] = "passed"
    except Exception as error:
        report["status"] = "failed"
        report["error_type"] = type(error).__name__  # No command output or unsanitized exception text.
    finally:
        ...
        save()
        print(json.dumps({"status": report["status"], "report": str(output / "acceptance.json"),
                          "temporary_workspace": str(workspace)}), flush=True)
